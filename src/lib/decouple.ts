...
export type GroupKey = string; // grouping key (e.g., resource)
...
function byCase(events: EventLogEvent[]): Map<string, EventLogEvent[]> {
  ...
  for (const [, arr] of m) arr.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
  return m;
}
...
export function decoupleByPathDownstream(
  ...
  return decoupleDownstreamBySelector(_graph, events, target, selectorFromPath(path));
}
...
export function decoupleNodeLocalByPath(
  ...
  return decoupleNodeLocalBySelector(_graph, events, target.id, selectorFromPath(path));
}
